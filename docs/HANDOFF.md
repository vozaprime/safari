...
12b. **Makale-içi görsel + medya kütüphanesi + görsel seçici (2026-08-12):** Zengin metin editörüne (`RichTextEditor`) "Görsel ekle" düğmesi eklendi; görseller makale metnine `![alt](url)` mini-markdown satırı olarak, imlecin olduğu yere, istenildiği kadar gömülür (şema değişmedi). Ortak render katmanı (`src/lib/richtext.tsx`) `image` blok türü kazandı → hizmet, blog ve içerik alanlarının hepsi tek seferde destekler. `ArticleBody` görselleri kırpmadan çizer; koddaki sabit orta-görsel (`inlineImages` haritası) artık YALNIZCA gövdede hiç görsel yoksa yedek olarak gösterilir. Yeniden kullanılabilir `MediaPicker` modalı (mevcutları göster + yeni yükle) tüm görsel alanlarına ("Kütüphaneden seç": hizmet banner'ı, referans logosu, blog kapağı, ayarlar logo/favicon) ve editöre bağlandı. Medya kütüphanesine (`/admin/media`) toplu/sürükle-bırak yükleme (`MediaUploader`) eklendi. Yeni: `src/lib/media.ts` (paylaşılan `listMedia`), `GET /api/admin/media`. Spec+plan: `docs/superpowers/{specs,plans}/2026-08-12-makale-gorsel-medya-kutuphanesi*`. 7 görev subagent-driven yürütüldü, her biri spec+kalite incelemesinden geçti; bütün-dal incelemesi "merge'e hazır". Doğrulama: `npm run lint` 0 + `npm run build` yeşil + `scripts/check-richtext-images.ts` + public regresyon (mevcut hizmet sayfası + eski yedek görsel sağlam). **Admin panel canlı tıklama testi kullanıcıya bırakıldı; henüz canlıya DEPLOY EDİLMEDİ.**

12c. **Hizmet + blog editoryal makale tasarımı (2026-08-12):** Claude Design mock'undan ("Hizmetler ve Blog") esinlenerek makale gövdesi yeni **`EditorialArticleBody`** (hizmet detay + blog yazısı ortak) ile editoryal düzene geçti: `##` başlıklar → numaralı bölüm (altın Playfair 01/02…+ çizgi), `>` alıntı → ortalanmış Playfair italik pull-quote (altın tırnak), `-` liste → altın-kare imler, `![görsel]` → kaydırılmış koyu-yeşil panel (yönü sırayla değişir). **Hero header'lar KORUNDU** (kullanıcı isteği: sadece makale bölümü yeniden tasarlandı) — hem hizmet forest hero'su hem blog hero'su aynen kaldı. Eski `ArticleBody` **kaldırıldı** (artık kullanılmıyor). Mevcut marka renkleri (forest/gold/ivory) + Inter korundu. Hizmette kapsam/CTA da editoryal restyle edildi.

12. **Güvenlik yükseltmesi (2026-08-06):** `npm audit`'teki 4 high açık kapatıldı → **0 açık**. İki commit: **nodemailer 7→9.0.4** (+@types 6→8; SMTP/CRLF enjeksiyonu) ve **Next.js 15→16.3.0** (+ react/react-dom 19.1→19.2.8; Next'in 8 açığı + bağımlı savunmasız postcss & sharp). Next 15.x hattında yama yoktu; tek çözüm Next 16 majör yükseltmesiydi. Kod uyumluydu: `params`/`searchParams`/`cookies()`/`headers()` zaten async, `next/image` hiç kullanılmıyor. Tek bilinçli davranış koruması: `<html>`'e `data-scroll-behavior="smooth"` (Next 16 SPA gezinmede scroll-behavior'ı ezmiyor; Lenis ile birlikte eski "anında yukarı" davranışı korundu). Turbopack artık varsayılan build. Doğrulandı: build başarılı + 3 dilde (tr/en/ru) 13 rota HTTP 200 + middleware admin kapısı (`/`→`/tr`, `/admin`→`/admin/login`) + sunucu/konsol hatasız. **main'e merge edildi (fast-forward).** Bağlı Next 16 temizlikleri aynı gün yapıldı: (a) `middleware.ts` → **`proxy.ts`** (fonksiyon `proxy`; deprecation giderildi, runtime edge→nodejs; §7); (b) `next lint` → **ESLint flat config** (`eslint`+`eslint-config-next`+`eslint.config.mjs`, `"lint":"eslint"`). Başlangıçtaki 15 react-hooks bulgusu (12 error/3 warning) da **düzeltildi**: temiz olanlar gerçek fix (immutability→index tabanlı, refs/purity→effect, prefer-const, unused-vars kaldırıldı), doğru-ama-yanlış-pozitif olanlar gerekçeli `eslint-disable` (setState-in-effect, server-component purity, no-img-element). `npm run lint` → **0** (ignore desenleri iç içe `.next`/`.claude` worktree kopyaları için sağlamlaştırıldı). **Canlıya deploy `npx vercel deploy --prod --yes` ile YAPILACAK** (DB şeması değişmedi; deploy öncesi iletişim formu maili + scrollytelling'i tarayıcıda gözden geçir).
...
- Makale-içi görseller: ~~sabit tek orta görsel~~ → **ÇÖZÜLDÜ (2026-08-12, §6.12b):** artık panelden metnin herhangi bir yerine sınırsız görsel eklenebiliyor. Koddaki `inlineImages` haritası yalnızca gövdede görsel olmayan eski hizmetler için yedek. İnceleme sonrası küçük iyileştirmeler de eklendi (2026-08-12): otomatik `alt` metnindeki rastgele dosya-adı eki temizlenir (rakam içeren 6+ karakterlik son token atılır, gerçek kelimeler korunur); `MediaUploader` dropzone klavye erişimli (role/tabIndex/Enter-Space); yükleme hataları ayrıntılı gösterilir (too_large/bad_type/unauthorized → Türkçe mesaj).
