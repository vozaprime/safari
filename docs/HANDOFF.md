...
12c. **Hizmet detay editoryal tasarımı (2026-08-12):** Claude Design mock'undan ("Hizmetler ve Blog") esinlenerek hizmet makale gövdesi yeni **`ServiceArticleBody`** ile editoryal düzene geçti: `##` başlıklar → numaralı bölüm (altın Playfair 01/02…+ çizgi), `>` alıntı → ortalanmış Playfair italik pull-quote (altın tırnak), `-` liste → altın-kare imler, `![görsel]` → kaydırılmış koyu-yeşil panel (yönü sırayla değişir). **Forest hero header KORUNDU** (kullanıcı isteği: sadece makale bölümü yeniden tasarlandı). Blog (`ArticleBody`) DEĞİŞMEDİ. Mevcut marka renkleri (forest/gold/ivory) + Inter korundu. Kapsam/CTA da editoryal restyle edildi.
